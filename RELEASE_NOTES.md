...
### BetterAngle Pro v5.5.184
- Automated build release.

### BetterAngle Pro v5.5.183
- **perf: Remove ineffective GDI sub-frame check + lower idleSamples threshold (v5.5.183).** `CheckTripwireGDI` was polling 3 pixels via GetPixel every 100?s between DXGI frames, but GetPixel reads from the DWM compositor buffer ? the exact same data source as DXGI Desktop Duplication. It has identical staleness and cannot detect a FOV change before DXGI does. Removed entirely, eliminating ~150?200?s of GDI overhead per poll and hundreds of redundant syscalls per frame gap. Separately, the tripwire idleSamples qualification gate lowered from 500 to 200 ? at ~60fps scanning this cuts the noise-estimation window from ~8s to ~3s of idle time, allowing the tripwire to arm fully within the first match after 3 skydives rather than needing prolonged idle periods.

### BetterAngle Pro v5.5.182
- **perf: Faster tripwire activation + tighter spin-wait (v5.5.182).** Two gap-closing optimizations: (1) Tripwire pre-arm now activates after 3 FOV-change events instead of 5 ? the 100% hit-rate and <0.1% noise-rate qualification gates are unchanged, so false-fire rate is unaffected; users get the fast 3-pixel pre-arm path after ~3 skydives instead of ~5. (2) BlockInput worker spin-wait tightened from 1ms (GetTickCount64, ~1ms resolution) to 200?s (QPC-timed, ~1?s resolution) ? saves ~0.5?0.8ms of post-BlockInput spinning while still catching back-to-back signals within the same DXGI frame window. QPC frequency cached once at thread start.

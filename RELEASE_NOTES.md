...
- **perf: Remove ineffective GDI sub-frame check + lower idleSamples threshold (v5.5.183).** `CheckTripwireGDI` was polling 3 pixels via GetPixel every 100µs between DXGI frames, but GetPixel reads from the DWM compositor buffer — the exact same data source as DXGI Desktop Duplication. It has identical staleness and cannot detect a FOV change before DXGI does. Removed entirely, eliminating ~150–200µs of GDI overhead per poll and hundreds of redundant syscalls per frame gap. Separately, the tripwire idleSamples qualification gate lowered from 500 to 200 — at ~60fps scanning this cuts the noise-estimation window from ~8s to ~3s of idle time, allowing the tripwire to arm fully within the first match after 3 skydives rather than needing prolonged idle periods.
